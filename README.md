...
Things planned, roughly in priority order. Open issues / PRs welcome on any of these.

### Auth & persistence
- **Redis-backed sessions, DCR clients, and OAuth tokens.** Today everything is in-memory — a container restart forces every Claude.ai connection to re-register and re-login. Moving session storage to Redis (`connector-redis` service in the compose, called via a small `Storage` interface) will make the server survive restarts cleanly.
- **Per-token scopes** once persistence lands — bearer tokens with `read-only` vs `read-news` vs `account-summary-only` so multiple callers can have different trust levels even though they all hit the same IBKR account.

### Instruments & exchanges
This server is currently US-stock-biased because that's what I trade. The catalog already includes scans for ETFs, options, futures, bonds, and several international exchanges — but the **example prompts, default `location` codes, and tool defaults** all assume `STK.US.MAJOR`. The plan is to:
- **Add first-class support for options, futures, bonds, and ETFs** — surface them as instrument choices in the tool docstrings, with sensible default `location` codes per instrument class.
- **Add European, Asian, and other major exchange routings** — `STK.EU`, `STK.HK`, `STK.JP`, etc. — with location-aware default filters.
- **Multi-currency portfolio rollups** — `ib_account_summary` will gain a base-currency conversion option once multi-currency cash is more common in test runs.

### Research depth
- **`ib_contract_details`** — fetch industry / category / subcategory / longName per ticker, so the LLM can do sector-level filtering on top of scan output. (Today, sector filtering relies on the LLM's general knowledge of company names — works but lossy.)
- **`ib_historical_data`** — pull short historical bars for candidate tickers so the LLM can sanity-check chart shape.
- **`ib_news_search`** — broader news search across symbols / keywords once a paid news provider is wired up. The plumbing is already in place; just needs entitlement.

### Quality of life
- **Knowledge-graph view of the catalog** — once the catalog stops growing (it shouldn't, much), a small Memgraph dataset linking `Strategy ↔ ScanCode ↔ Instrument ↔ Vendor ↔ Entitlement` so multi-hop questions become tractable (*"give me value scans on STK that don't require Refinitiv entitlement"*).
- **Auto-regenerate the catalog** when IBKR ships new scan codes — today it's manual via `scripts/build_catalog.py`. Move it into the image build.
- **Observability** — structured JSON logs of tool calls (without token bodies) and basic Prometheus counters so you can see which scans your LLMs are actually using.
